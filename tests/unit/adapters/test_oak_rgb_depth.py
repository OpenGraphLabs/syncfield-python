"""Unit tests for OakRgbDepthStream using a mocked depthai module."""
...
def mock_depthai(monkeypatch):
    ...
    sys.modules.pop("syncfield.adapters.oak_rgb_depth", None)
    yield fake
    sys.modules.pop("syncfield.adapters.oak_rgb_depth", None)
...
class TestCapabilities:
    def test_capabilities(self, mock_depthai, mock_av_generous, tmp_path):
        ...
        stream = OakRgbDepthStream("oak", output_dir=tmp_path)
...
class TestDepthOption:
    ...
    def test_depth_disabled_by_default(
        ...
        stream = OakRgbDepthStream("oak", output_dir=tmp_path, encoding="raw")
        stream.prepare()
        stream.connect()
        try:
            pipeline = fake.Pipeline.return_value
...
class TestEncoding:
    ...
    def test_invalid_encoding_raises(self, mock_depthai, mock_av_generous, tmp_path):
        ...
        with pytest.raises(ValueError, match="encoding"):
            OakRgbDepthStream("oak", output_dir=tmp_path, encoding="vp9")
...
class TestImportGuard:
    def test_depthai_missing_raises_clear_install_hint(self, monkeypatch):
        ...
        with pytest.raises(ImportError, match=r"syncfield\[oak\]"):
            importlib.import_module("syncfield.adapters.oak_rgb_depth")
...
class TestDeviceShutterHostNs:
    ...
    def _load_helper(self, mock_depthai):  # noqa: ARG002 - fixture runs for side effects
        from syncfield.adapters.oak_rgb_depth import _device_shutter_host_ns
...
class TestRecordingAnchor:
    ...
    def test_anchor_captured_from_first_frame(
        ...
        from syncfield.adapters import oak_rgb_depth as oak_camera
        from syncfield.adapters.oak_rgb_depth import OakRgbDepthStream
