"""OakRgbDepthStream — DepthAI-based reference adapter for Luxonis OAK cameras.
...
try:
    ...
    raise ImportError(
        "OakRgbDepthStream requires depthai. "
...
class OakRgbDepthStream(StreamBase):
    ...
    * ``disconnect()`` tears down the pipeline.
    ...
    _discovery_adapter_type = "oak_rgb_depth"
    ...
    def __init__(
        ...
        if encoding not in _OAK_ENCODINGS:
            raise ValueError(
                f"OakRgbDepthStream.encoding must be one of {_OAK_ENCODINGS!r}, "
                f"got {encoding!r}"
            )
...
def iter_depth_frames(
    ...
    Args:
        path: Path to the ``.depth.bin`` file produced by OakRgbDepthStream.
